...
public class AlarmReceiver2 extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        builder.setAutoCancel(true).setDefaults(NotificationCompat.DEFAULT_ALL)
                .setContentTitle("10분 후 알람을 알려드립니다.")
                .setContentText("10분마다 알람이 울립니다.")
        ...
        Intent i = new Intent(context, AlarmReceiver2.class);
        PendingIntent pendingIntent = PendingIntent.getBroadcast(context, 20, i, PendingIntent.FLAG_UPDATE_CURRENT);

        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            alarmManager.setExactAndAllowWhileIdle(AlarmManager.RTC_WAKEUP, System.currentTimeMillis() + (60 * 1000 * 10), pendingIntent);
        } else {
            alarmManager.setExact(AlarmManager.RTC_WAKEUP, System.currentTimeMillis() + (60 * 1000 * 10), pendingIntent);
        }
    }
}
